---
layout: post
title:  "Node.js 1 - 기본 기능 & http 모듈"
date:   2019-07-28 02:02:59
author: Inhyuk
categories: NodeJS
tags: NodeJS
cover:  "/assets/instacode.png"
name: 2019-07-28-nodejs1.md
---
...
- os : 시스템 정보를 알려주는 모듈.
1. hostname() : 운영체제의 호스트 이름
2. totalmem() : 시스템 전체의 메모리 용량
3. freemem() : 사용 가능한 메모리 용량
4. cpus() : cpu 정보
5. networkInterfaces() : 네트워크 인터페이스를 담은 배열

- path : 파일 경로를 다루는 모듈
1. join() : 여러개의 이름을 합쳐서 하나의 파일 경로를 만든다
2. dirname() :디렉토리 이름 반환
3. basename() : 확장자 이름을 제외한 파일 경로 반환
4. extname() : 파일 확장자를 반환
...
1. parse() : 주소문자열을 파싱하여 url 객체 생성
2. format() : URL 객체를 주소 문자열로 파싱

- querystring 모듈 : 요청 parameter를 분류하는 데에 사용
1. parse() : 요청 parameter를 파싱하여 객체로 생성
2. stringify() : 객체를 문자열로 변환
...
- server의 이벤트 :
1. request : 클라이언트의 요청시 발생하는 이벤트. 별도로 지정하지 않고 createServer 의 callback으로 이벤트 리스너 등록 가능.
1. connection : 클라이언트가 접속할 때
1. close : 서버가 종료될 때
2. checkContinue : 클라이언트가 지속적으로 연결을 하고 있을 때
3. upgrade : 클라이언트가 http 업그레이드를 요청할 때
4. clientError : 클라이언트가 오류가 발생할 때
...
1. 1xx : 처리 중
2. 2xx : 성공
3. 3xx : 리다이렉트(다른 곳으로 연결)
4. 4xx : 클라이언트 오류. 404(page not found)
5. 5xx : 서버 오류
...
3. text/css : css 문서
4. text/xml : xml 문서
5. image/jpeg : jpg/jpeg 그림
1. image/png : png 그림
1. video/mpeg : mpeg 비디오
1. audio/mp3 : mp3 음악파일
...
2. url : 요청한 url
1. headers : 요청 메시지 헤더
1. trailers : 요청 메시지 트레일러
1. httpVersion : http 프로토콜 버전
