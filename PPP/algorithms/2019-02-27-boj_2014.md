...
하지만 가정에 의해 b는 이미 구한 수이고 우리의 계산법에 의하면 $b*p$를 놓칠 수는 없다. 그러므로 $a$는 빠질 수가 없다.

##### 중복 제거

모든 수를 넣는다고 해서 해결되는 문제가 아니다.\\
수를 넣기만 하면 메모리가 기하급수적으로 증가할 것이다. 또한, *heap* 에서 이 수가 중복된 수 인지 아닌지를 가늠하기도 힘들 것이다.
...
언틋보기에도 순열(Permutation)임을 알 수 있다.\\
그러나 사실 우리가 원하는 것은 곱셈이므로 일종의 조합(Combination)이라고 볼 수 있다.\\
...
항상 *heap* 이 꽉차있다고 하더라도, *heap* 의 크기를 k로 제한했기 때문에 시간 복잡도는 $O(klogk)$이다.\\
각 반복마다 n개의 수와 곱해지는 과정이 존재하지만 N의 크기가 매우 작으므로 무시해도 된다.
