...
$$
경로 p: E의 부분집합 {e_1,..,e_n}의 나열한 것 중 하나로,\\ e_1=(v_i,v_{k_1}),e_2=(v_{k_1},v_{k_2}),...,e_n=(v_{k_{n-1}},v_j)을 만족하고,\\
모든 v들(시작과 끝은 같아도 상관 없음)은 서로 다르며,\\
path에 속한 모든 edge들이 서로 다를 때, p를 v_i에서 v_j로의 경로라고 부른다.
$$
...
* 인접리스트는 **sparse** 한 경우에 적은 메모리로 저장을 할 수 있지만, 두 노드 사이의 edge가 존재하는 지를 확인하기 위하여 최악의 경우 시간이 $O(N) , N:#nodes$만큼 걸릴 수가 있다.
...
3. M에 들어있는 node들과 $M$에 속하지 않는 node들 사이의 edge중 weight가 가장 작은 edge를 선택하고, 그 edge의 두 node 중 $M$에 속하지 않은 node를 M에 추가한다.
